...
- (void)handleDeviceOrientationDidChange:(NSNotification *)notification {
    ...
    if (isLandscape) {
        CGFloat delta = (UIDeviceOrientationLandscapeLeft == orientation) ? 1 : -1;
        transform = CGAffineTransformMakeRotation(delta * M_PI / 2.);
    }
    
    [[UIApplication sharedApplication] setStatusBarHidden:isLandscape
        ...
        self.view.frame = [UIScreen mainScreen].bounds;
                     ...
                     completion:^(BOOL finished) {
                         if (finished) {
                             ...
                             /// 因为弹出的是竖屏的, 所以在横屏隐藏.
                             UIColor *tintColor = isLandscape ? [UIColor clearColor] : [[UIBarButtonItem appearanceWhenContainedIn:[self class], nil] tintColor];
                             
                             _deleteButton.enabled = !isLandscape;
                             _deleteButton.tintColor = tintColor;
                             
                             _actionButton.enabled = !isLandscape;
